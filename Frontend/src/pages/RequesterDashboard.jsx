...
import { Radio, Plus, Clock, MapPin, CheckCircle2, AlertTriangle, UserCheck, RefreshCw, ChevronRight, Phone, MessageSquare } from 'lucide-react';

export default function RequesterDashboard({ setActiveTab }) {
  const { setSosModalOpen, requests, renewRequest, user } = useCrisis();
  const [selectedReqId, setSelectedReqId] = useState(null);

  // Filter requests submitted by requester
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 bg-gradient-to-r from-slate-900 via-slate-900 to-red-950/40 relative overflow-hidden">
        <div className="max-w-xl space-y-2">
          ...
          <p className="text-sm text-slate-300">
            If you are in immediate life-threatening danger, tap the SOS button below. For non-critical support, create a standard help request.
          </p>
        </div>
      </div>
      ...
      <div className="glass-panel p-8 rounded-3xl border-2 border-red-600/50 bg-gradient-to-b from-red-950/60 to-slate-950 text-center space-y-6 shadow-2xl relative">
        <div className="mx-auto w-24 h-24 rounded-full bg-red-600 border-4 border-red-400/40 flex items-center justify-center cursor-pointer hover:scale-105 transition-transform sos-pulse-ring shadow-2xl shadow-red-900" onClick={() => setSosModalOpen(true)}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {myRequests.map(req => (
            ...
            >
              ...
              {/* AUTO-EXPIRY & RENEWAL CARD */}
              <div className="flex items-center justify-between pt-2 border-t border-slate-800 text-xs">
                <div className="flex items-center gap-1.5 text-slate-400">
                  <Clock className="w-4 h-4 text-amber-500" />
                  <span>Auto-Expires in: <strong className="text-amber-300 font-mono">02:34:18</strong></span>
                </div>

                <button
                  onClick={() => renewRequest(req.id)}
                  className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg font-semibold flex items-center gap-1 text-xs border border-slate-700"
                >
              ...
              <button
                ...
                className="w-full py-2 bg-slate-900 hover:bg-slate-800 text-slate-300 rounded-xl text-xs font-semibold flex items-center justify-center gap-1 border border-slate-800"
              >
